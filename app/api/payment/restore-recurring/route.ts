...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: sub } = await (supabase as any)
      ...
      .select('id, is_recurring, subscription_type, status, package:subscription_packages(month, year)')
    ...
    // Notify admins on their Messages page that this student restored recurring.
    // Best-effort — never fail the restore itself if the notification insert has an issue.
    try {
      ...
      await (admin as any).from('admin_notifications').insert({
        ...
        message: `${name} of ${gradeName} has restored their recurring monthly live subscription for ${monthLabel}.`,
        student_id: user.id,
        grade_id: prof?.grade_id ?? null,
      })
    } catch (e) {
      console.error('[payment/restore-recurring] notification insert failed:', e)
    }
