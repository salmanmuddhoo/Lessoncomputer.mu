...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error('[payment/create] env check', {
      ...
      MIPS_BASIC_USER:         process.env.MIPS_BASIC_USER         ? `set(${process.env.MIPS_BASIC_USER.length})`         : 'fallback to id_operator',
      MIPS_BASIC_PASS:         process.env.MIPS_BASIC_PASS         ? `set(${process.env.MIPS_BASIC_PASS.length})`         : 'fallback to operator_password',
